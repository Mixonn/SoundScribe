...
export default class PlaybackEngine {
  ...
  async stop () {
    ...
    if (this.playbackSettings.instrument) { await this.playbackSettings.instrument.stop() }
    this._clearTimeouts()
  ...
  setVoiceVolume (instrumentId, voiceId, volume) {
    const playbackInstrument = this.playbackSettings.volumes.instruments.find(
      i => i.id === instrumentId
    )
    const playbackVoice = playbackInstrument.voices.find(v => v.id === voiceId)
  ...
  _notePlaybackCallback (audioDelay, notes) {
    ...
    for (const note of notes) {
      const noteDuration = this._getNoteDuration(note)
      if (noteDuration === 0) { continue }
      const noteVolume = this._getNoteVolume(note)
  ...
  _getNoteVolume (note) {
    const instrument = note.voiceEntry.ParentVoice.Parent
    const playbackInstrument = this.playbackSettings.volumes.instruments.find(
      i => i.id === instrument.Id
    )
    const playbackVoice = playbackInstrument.voices.find(
